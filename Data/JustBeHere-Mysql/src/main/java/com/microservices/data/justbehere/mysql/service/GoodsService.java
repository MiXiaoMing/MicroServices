...
public class GoodsService {
     ...
     */
    public GoodsCollection selectFromCache(String code) {
        ResponseModel<ExtendResult> responseModel = dataCacheClient.getExtend(PRE + code);
        if (responseModel.isSuccess()) {
            sendMQ(code, responseModel.getData().value);

            return JSONObject.parseObject(responseModel.getData().value, GoodsCollection.class);
        }
        return null;
    }
     ...
     * @return
     */
    public GoodsCollection selectFromMysql(String code) {
        Map<String, Object> map = new HashMap<>();
        map.put("code", code);
        map.put("carouselCode", Constants.carousel_goods);

        GoodsCollection entity = sqlSessionTemplate.selectOne("com.microservices.data.justbehere.mysql.GoodsMapper.selectCollection", map);
        if (entity != null) {
