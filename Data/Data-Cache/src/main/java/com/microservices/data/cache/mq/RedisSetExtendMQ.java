...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(value = "redis-setExtend", autoDelete = "false"),
                exchange = @Exchange(value = "redis", type = ExchangeTypes.DIRECT),
                key = "setExtend"
        )
)
public class RedisSetExtendMQ {

    @Autowired
    private RedisService redisService;

    private final Logger logger = LoggerFactory.getLogger(RedisSetExtendMQ.class);
